...
class DataWrangler:
    ...
    def remove_missing(self, nullRatios, drop_col_ratio):
        ...
        for k, v in nullRatios.items():
            ...
            print(k)
            # if the value of the ration is larger than the min percentage drop the column
            if v > minPercent:
                self.data.drop(str(k), axis=1, inplace=True)
                #print(df.columns.values)
            #if the type is np.object or string remove
    ...
    def remove_unnecesary_rows(self):
      ...
      cleaned_data = self.data.drop(self.unnecesary_cols, axis=1, inplace=True)
