...
class ColumnSelector:
    ...
    def select_processing_data(self, pred_column):
        """Selects X and y"""
        #check if pred column is in the columns value

        self.data.reset_index(drop=True, inplace=True)
        if pred_column in self.data.columns:
            if pred_column in self.cat_cols or pred_column in self.num_cols:

                #self.data.reset_index(drop=True, inplace=True)
                self.y = self.data[pred_column]#.tolist()
    ...
    def modelling_columns(self, pred_column):
        ...
        if pred_column in self.cat_cols:
            ...
            cat_cols = self.cat_cols.remove(pred_column)
